...
// ==================== دالة مساعدة لقراءة المتغيرات البيئية بأسماء متعددة ====================
...
// ==================== اتصال قاعدة البيانات (مع SSL ومرونة الأسماء) ====================
const dbConfig = {
    ...
    // تفعيل SSL إذا كان المتغير موجوداً أو إذا كان المضيف من Aiven أو Railway
    ssl: (process.env.DB_SSL === 'true' || process.env.MYSQL_SSL === 'true' ||
        ...
        ? { rejectUnauthorized: false } // السماح بالشهادات الذاتية (آمن في Aiven/Railway)
        : false
};

console.log('🔌 محاولة الاتصال بقاعدة البيانات بالتهيئة:');
...
console.log(`   SSL: ${dbConfig.ssl ? 'مفعل (بدون التحقق من الشهادة)' : 'غير مفعل'}`);
...
db.connect((err) => {
    if (err) {
        ...
        console.error('   - تأكد من صحة متغيرات البيئة (DB_HOST, DB_USER, إلخ)');
        console.error('   - في Railway، استخدم المتغيرات المولدة تلقائياً (MYSQL_HOST, MYSQL_USER, إلخ)');
