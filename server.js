...
// جميع نقاط النهاية في ملف واحد، يدعم نظام المخزون والمعاملات
// ================================================================
...
db.connect((err) => {
    if (err) {
        ...
        console.error('❌ تأكد من إعداد متغيرات البيئة في Railway (DB_HOST, DB_USER, DB_PASSWORD, DB_NAME)');
        process.exit(1);
    }
...
const authenticateAdmin = (req, res, next) => {
    ...
    if (!authHeader) return res.status(401).json({ status: false, message: 'غير مصرح به' });
    const token = authHeader.split(' ')[1];
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        db.query('SELECT id FROM admins WHERE id = ?', [decoded.id], (err, results) => {
            if (err || results.length === 0) return res.status(403).json({ status: false, message: 'صلاحية غير كافية' });
    ...
    db.beginTransaction((err) => {
        ...
        db.query('SELECT * FROM categories WHERE name = ? OR id = ? FOR UPDATE', [category, category], (err, catResults) => {
            ...
            db.query('SELECT COUNT(*) as available_count FROM cards WHERE category = ? AND user_id IS NULL AND status = "available"', [category], (err, stockResult) => {
                ...
                if (availableCount < quantity) return db.rollback(() => res.status(400).json({ status: false, message: `لا يوجد كروت كافية من فئة "${category}". المتاح: ${availableCount}، المطلوب: ${quantity}` }));
                db.query('SELECT balance FROM users WHERE id = ? FOR UPDATE', [userId], (err, userResults) => {
                    ...
                    db.query('UPDATE users SET balance = ? WHERE id = ?', [newBalance, userId], (err) => {
                        ...
                        db.query('UPDATE cards SET user_id = ?, sold_at = NOW(), status = "sold", receiver_phone = ? WHERE category = ? AND user_id IS NULL AND status = "available" LIMIT ?',
                            ...
                            (err, result) => {
                                if (err) return db.rollback(() => { console.error('خطأ في حجز الكروت:', err); res.status(500).json({ status: false, message: 'فشل حجز الكروت، تم التراجع' }); });
                                if (result.affectedRows < quantity) return db.rollback(() => res.status(400).json({ status: false, message: `تم حجز ${result.affectedRows} كرت فقط، المطلوب ${quantity}` }));
...
app.post('/api/transfer', async (req, res) => {
    ...
    db.beginTransaction((err) => {
        ...
        db.query('SELECT id, name, balance FROM users WHERE id = ? FOR UPDATE', [senderId], (err, senderResults) => {
            if (err) return db.rollback(() => res.status(500).json({ status: false, message: 'خطأ في قاعدة البيانات' }));
            if (senderResults.length === 0) return db.rollback(() => res.status(404).json({ status: false, message: 'المستخدم المرسل غير موجود' }));
            ...
            db.query('SELECT id, name FROM users WHERE account_number = ? FOR UPDATE', [receiverAccount], (err, receiverResults) => {
                if (err) return db.rollback(() => res.status(500).json({ status: false, message: 'خطأ في قاعدة البيانات' }));
                if (receiverResults.length === 0) return db.rollback(() => res.status(404).json({ status: false, message: 'المستقبل غير موجود (رقم حساب خاطئ)' }));
                ...
                db.query('UPDATE users SET balance = balance - ? WHERE id = ?', [amount, senderId], (err) => {
                    ...
                    db.query('UPDATE users SET balance = balance + ? WHERE id = ?', [amount, receiver.id], (err) => {
                        ...
                        db.query('INSERT INTO transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())',
                            [senderId, -amount, 'transfer', `تحويل إلى ${receiver.name} (رقم حساب ${receiverAccount})${note ? ' - ' + note : ''}`],
                            (err) => {
                                if (err) return db.rollback(() => res.status(500).json({ status: false, message: 'فشل تسجيل معاملة المرسل' }));
                                db.query('INSERT INTO transactions (user_id, amount, type, description, created_at) VALUES (?, ?, ?, ?, NOW())',
                                    [receiver.id, amount, 'transfer', `استلام تحويل من ${sender.name} (رقم حساب)${note ? ' - ' + note : ''}`],
                                    (err) => {
...
app.post('/api/send-whatsapp', (req, res) => {
    ...
    Promise.all(promises).then(() => res.status(200).json({ status: true, message: `تم إرسال ${cardCodes.length} كود إلى ${phone} عبر واتساب (محاكاة)` }))
...
// ================================================================
// 2. نقاط نهاية الإدارة (محمية بـ authenticateAdmin)
// ================================================================
...
app.get('/api/admin/users/:id', authenticateAdmin, (req, res) => {
    db.query('SELECT id, name, phone, balance, account_number, status FROM users WHERE id = ?', [req.params.id], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'خطأ في قاعدة البيانات' });
...
app.post('/api/admin/users', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        db.query('INSERT INTO users (name, phone, password, balance, account_number, status) VALUES (?, ?, ?, ?, ?, ?)',
            [name, phone, hashed, balance || 0, accountNumber, status || 'active'],
            (err, result) => {
                if (err) return res.status(500).json({ status: false, message: 'فشل إضافة المستخدم' });
...
app.put('/api/admin/users/:id', authenticateAdmin, async (req, res) => {
    ...
    db.query(sql, params, (err) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل تحديث المستخدم' });
...
app.put('/api/admin/users/:id/toggle-status', authenticateAdmin, (req, res) => {
    db.query('SELECT status FROM users WHERE id = ?', [req.params.id], (err, result) => {
        ...
        db.query('UPDATE users SET status = ? WHERE id = ?', [newStatus, req.params.id], (err) => {
            if (err) return res.status(500).json({ status: false, message: 'فشل تغيير الحالة' });
    ...
    db.beginTransaction((err) => {
        ...
        db.query('UPDATE users SET balance = balance + ? WHERE id = ?', [amount, req.params.id], (err, result) => {
            ...
            db.query('INSERT INTO transactions (user_id, amount, type, description) VALUES (?, ?, "recharge", ?)', [req.params.id, amount, note || 'شحن رصيد من لوحة الإدارة'], (err) => {
                if (err) return db.rollback(() => res.status(500).json({ status: false, message: 'فشل تسجيل المعاملة' }));
                db.commit(() => res.json({ status: true, message: `تم شحن ${amount} ر.ي بنجاح للمستخدم` }));
            });
        });
    });
});
app.delete('/api/admin/users/:id', authenticateAdmin, (req, res) => {
    db.query('DELETE FROM users WHERE id = ?', [req.params.id], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل حذف المستخدم' });
...
app.post('/api/admin/cards', authenticateAdmin, (req, res) => {
    ...
    db.query('INSERT INTO cards (category, card_code, status, receiver_phone) VALUES (?, ?, ?, ?)', [category, cardCode, status || 'available', receiver_phone || null], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل إضافة الكرت' });
...
app.post('/api/admin/cards/bulk', authenticateAdmin, (req, res) => {
    ...
    if (!category || !count || count < 1) return res.status(400).json({ status: false, message: 'الفئة والعدد المطلوبين (العدد > 0)' });
    const numCount = parseInt(count);
    if (isNaN(numCount) || numCount < 1) return res.status(400).json({ status: false, message: 'العدد يجب أن يكون رقماً صحيحاً موجباً' });
    const values = [];
    for (let i = 0; i < numCount; i++) { values.push([category, `WIFI-${Date.now()}-${Math.floor(Math.random() * 100000)}-${i}`, 'available', new Date()]); }
    db.query('INSERT INTO cards (category, card_code, status, created_at) VALUES ?', [values], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل إضافة الدفعة' });
...
app.post('/api/admin/cards/bulk-import', authenticateAdmin, (req, res) => {
    ...
    if (cleanCodes.length === 0) return res.status(400).json({ status: false, message: 'لا توجد أكواد صالحة للإضافة' });
    const values = cleanCodes.map(code => [category, code, 'available', new Date()]);
    db.query('INSERT INTO cards (category, card_code, status, created_at) VALUES ?', [values], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل إضافة الكروت: ' + err.message });
...
app.put('/api/admin/cards/:id', authenticateAdmin, (req, res) => {
    const { category, status, receiver_phone } = req.body;
    db.query('UPDATE cards SET category = ?, status = ?, receiver_phone = ? WHERE id = ?', [category, status || 'available', receiver_phone || null, req.params.id], (err) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل تحديث الكرت' });
...
app.delete('/api/admin/cards/:id', authenticateAdmin, (req, res) => {
    db.query('DELETE FROM cards WHERE id = ?', [req.params.id], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل حذف الكرت' });
        if (result.affectedRows === 0) return res.status(404).json({ status: false, message: 'الكرت غير موجود' });
        res.json({ status: true, message: 'تم حذف الكرت بنجاح' });
    });
});
app.post('/api/admin/cards/bulk-delete', authenticateAdmin, (req, res) => {
    ...
    if (!cardIds || !Array.isArray(cardIds) || cardIds.length === 0) return res.status(400).json({ status: false, message: 'يرجى تحديد الكروت المراد حذفها' });
    const placeholders = cardIds.map(() => '?').join(',');
    db.query(`DELETE FROM cards WHERE id IN (${placeholders})`, cardIds, (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل حذف الكروت المحددة' });
...
app.delete('/api/admin/cards/all', authenticateAdmin, (req, res) => {
    db.query('DELETE FROM cards', (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل حذف جميع الكروت' });
        res.json({ status: true, message: `تم حذف ${result.affectedRows} كرت بنجاح` });
    });
});
...
app.get('/api/admin/categories', authenticateAdmin, (req, res) => {
    db.query('SELECT c.*, (SELECT COUNT(*) FROM cards WHERE category = c.name) as card_count FROM categories c ORDER BY c.id DESC', (err, results) => {
        if (err) return res.status(500).json([]);
        res.json(results);
    });
});
...
app.post('/api/admin/categories', authenticateAdmin, (req, res) => {
    ...
    if (!name || price === undefined || price < 0) return res.status(400).json({ status: false, message: 'الاسم والسعر مطلوبان (السعر >= 0)' });
    db.query('INSERT INTO categories (name, price) VALUES (?, ?)', [name, price], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل إضافة الفئة' });
...
app.put('/api/admin/categories/:id', authenticateAdmin, (req, res) => {
    const { name, price } = req.body;
    if (!name || price === undefined || price < 0) return res.status(400).json({ status: false, message: 'الاسم والسعر مطلوبان (السعر >= 0)' });
    db.query('UPDATE categories SET name = ?, price = ? WHERE id = ?', [name, price, req.params.id], (err) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل تحديث الفئة' });
    ...
    db.query('SELECT name FROM categories WHERE id = ?', [req.params.id], (err, result) => {
        ...
        db.query('DELETE FROM cards WHERE category = ?', [categoryName], (err) => {
            ...
            db.query('DELETE FROM categories WHERE id = ?', [req.params.id], (err) => {
                if (err) return res.status(500).json({ status: false, message: 'فشل حذف الفئة' });
...
app.get('/api/admin/transactions/export', authenticateAdmin, (req, res) => {
    db.query('SELECT t.*, u.name as user_name FROM transactions t LEFT JOIN users u ON t.user_id = u.id ORDER BY t.created_at DESC LIMIT 1000', (err, results) => {
        if (err) return res.status(500).json([]);
        res.json(results);
    });
});
app.delete('/api/admin/transactions/all', authenticateAdmin, (req, res) => {
    db.query('DELETE FROM transactions', (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل حذف المعاملات' });
...
app.get('/api/admin/admins/:id', authenticateAdmin, (req, res) => {
    db.query('SELECT id, username FROM admins WHERE id = ?', [req.params.id], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'خطأ في قاعدة البيانات' });
...
app.post('/api/admin/admins', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        db.query('INSERT INTO admins (username, password) VALUES (?, ?)', [username, hashed], (err, result) => {
            if (err) return res.status(500).json({ status: false, message: 'فشل إضافة المدير' });
...
app.put('/api/admin/admins/:id', authenticateAdmin, async (req, res) => {
    ...
    db.query(sql, params, (err) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل تحديث المدير' });
...
app.delete('/api/admin/admins/:id', authenticateAdmin, (req, res) => {
    ...
    db.query('DELETE FROM admins WHERE id = ?', [id], (err, result) => {
        if (err) return res.status(500).json({ status: false, message: 'فشل حذف المدير' });
        if (result.affectedRows === 0) return res.status(404).json({ status: false, message: 'المدير غير موجود' });
